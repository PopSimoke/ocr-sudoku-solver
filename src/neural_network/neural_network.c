...
void train(Neural_Network *nn, const double *inputs, const double *targets,
    ...
    Matrix *inputs_matrix = create_new_matrix_from_array(inputs, nn->inputLayerSize);
    Matrix *targets_matrix = create_new_matrix_from_array(targets, nn->outputLayerSize);
...
Neural_Network *load_neural_network(char *path, double learning_rate, size_t inputLayerSize, size_t outputLayerSize, size_t hiddenLayerSize) {
    ...
    if (fptr == NULL) {
      return create_new_neural_network(learning_rate, inputLayerSize, outputLayerSize, hiddenLayerSize);
    }
