...
void matrix_dot_multiply(Matrix *m1, Matrix *m2, Matrix *result) {
  ...
  if (m1->column_count != m2->line_count)
    errx(1,
         "Can't dot multiply two matrix of incompatible dimension: m1(%lu,%lu) "
         "and m2(%lu,%lu)",
  ...
  for (size_t line = 0; line < m1->line_count; line++)
    for (size_t column = 0; column < m2->column_count; column++) {
      ...
      for (size_t i = 0; i < m1->column_count; i++)
        sum += *get_matrix_element(m1, line, i) *
               *get_matrix_element(m2, i, column);
...
void matrix_multiply(Matrix *m1, Matrix *m2, Matrix *result) {
  ...
  if (m1->line_count != m2->line_count || m1->column_count != m2->column_count)
    errx(1,
         "Can't multiply two matrix of different dimensions: m1(%lu,%lu) and "
         "m2(%lu,%lu)",
         m1->line_count, m1->column_count, m2->line_count, m2->column_count);
...
void matrix_add(Matrix *m1, Matrix *m2, Matrix *result) {
  ...
  if (m1->line_count != m2->line_count || m1->column_count != m2->column_count)
    errx(1,
         "Can't add two matrix of different dimensions: m1(%lu,%lu) and "
         "m2(%lu,%lu)",
         m1->line_count, m1->column_count, m2->line_count, m2->column_count);
...
void matrix_subtract(Matrix *m1, Matrix *m2, Matrix *result) {
  ...
  if (m1->line_count != m2->line_count || m1->column_count != m2->column_count)
    errx(1,
         "Can't subtract two matrix of different dimensions: m1(%lu,%lu) and "
         "m2(%lu,%lu)",
         m1->line_count, m1->column_count, m2->line_count, m2->column_count);
...
void matrix_copy(Matrix *m1, Matrix *m2) {
  ...
  if (m1->line_count != m2->line_count || m1->column_count != m2->column_count)
    errx(1,
         "Can't copy a matrix to another one with different dimensions: "
         "m1(%lu,%lu) and m2(%lu,%lu)",
         m1->line_count, m1->column_count, m2->line_count, m2->column_count);
...
void set_identity_matrix(Matrix *m) {
  ...
  for (size_t i = 0; i < m->line_count * m->column_count; i++)
    m->data[i] = 0;
